...
where F: FnOnce(P::Output) -> Result<Output, P::Error> {
    type Output = Output;
    type Error = P::Error;
...
where F: Fn(P::Output) -> Result<Output, P::Error> {
...
where F: FnOnce(P::Error) -> Result<P::Output, Error> {
...
where F: Fn(P::Error) -> Result<P::Output, Error> {
    fn parse(&self, input: Input) -> ParseResult<Input, Self> {
        let (input, out) = self.0.parse(input);
        (input, out.or_else(&self.1))
    }
...
where F: FnOnce(P::Output) -> Result<Output, Error>,
...
where F: Fn(P::Output) -> Result<Output, Error>,
