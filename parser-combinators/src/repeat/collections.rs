...
where for<'a> &'a C: Collection<Item> {
    #[inline]
    fn put(&mut self, item: Item) { <&C>::put(&mut &**self, item); }
...
impl<Item, C: Default + Collection<Item>> Collection<Item> for Cell<C> {
    #[inline]
    fn put(&mut self, item: Item) { C::put(self.get_mut(), item); }
...
impl<Item, C: ?Sized + Collection<Item>> Collection<Item> for RefCell<C> {
    ...
    #[inline]
    fn is_empty(&self) -> bool { C::is_empty(&self.borrow()) }
    
    #[inline]
    fn len(&self) -> usize { C::len(&self.borrow()) }
}

impl<Item, C: ?Sized + Collection<Item>> Collection<Item> for &RefCell<C> {
    #[inline]
    fn put(&mut self, item: Item) { C::put(&mut self.borrow_mut(), item); }
...
impl<Item, C: Collection<Item>> Collection<Item> for RwLock<C> {
    #[inline]
    fn put(&mut self, item: Item) { C::put(self.get_mut().unwrap(), item); }
    
    #[inline]
    fn is_empty(&self) -> bool { C::is_empty(&self.read().unwrap()) }
    
    #[inline]
    fn len(&self) -> usize { C::len(&self.read().unwrap()) }
}

impl<Item, C: Collection<Item>> Collection<Item> for &RwLock<C> {
    #[inline]
    fn put(&mut self, item: Item) { C::put(&mut self.write().unwrap(), item); }
...
impl<Item, C: Collection<Item>> Collection<Item> for Mutex<C> {
    ...
    #[inline]
    fn is_empty(&self) -> bool { C::is_empty(&self.lock().unwrap()) }
    
    #[inline]
    fn len(&self) -> usize { C::len(&self.lock().unwrap()) }
}

impl<Item, C: Collection<Item>> Collection<Item> for &Mutex<C> {
    #[inline]
    fn put(&mut self, item: Item) { C::put(&mut self.lock().unwrap(), item); }
